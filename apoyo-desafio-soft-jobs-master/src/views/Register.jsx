...
const Register = () => {
  ...
  const handleForm = (event) => {
    ...
    if (user.password.length < 6) {
      return window.alert('La contraseña debe tener al menos 6 caracteres.')
    }
    ...
    axios.post(ENDPOINT.register, user)
      ...
      .catch((error) => {
        if (error.response) {
          ...
          window.alert(`${error.response.data.message || 'Error en el registro.'} 🙁.`)
        } else if (error.request) {
          console.error('Request error:', error.request)
          window.alert('Hubo un problema con la solicitud. Intenta más tarde.')
        } else {
